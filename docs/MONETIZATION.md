...
현재 서버는 `session_start`, `tutorial_complete`, `operation_complete`, `store_view`, `checkout_intent`, `purchase_complete`를 `analytics_events`에 기록합니다.
...
- v1.2 비공개 알파에서는 딥 토크·기억·안전 기능을 과금 등급으로 나누지 않고 호출 비용과 만족도부터 측정합니다.
